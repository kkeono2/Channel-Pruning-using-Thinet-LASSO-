...
def count_model_param_nums(model):
    total = sum([param.nelement() if param.requires_grad else 0 for param in model.parameters()])
    print('  + Number of params: %.4fM' % (total / 1e6))
    return total


def count_model_flops(model=None, input_res=224, multiply_adds=False):
    ...
    foo(model)
    inputs = torch.rand(3, input_res, input_res).unsqueeze(0)
